  ...
  for (int i=0;i<image_names.length;i++) {
    ...
    try {
      ...
      if (extension.equals(".jpg")) {
        println(image_names[i]+"["+str(i)+"/"+str(image_names.length)+"]");
        ...
        //create a standalone face file
        JSONObject image_face_data = new JSONObject();
        image_face_data.setJSONArray(image_names[i], image_faces);
        saveJSONObject(image_face_data, data_dir+"/"+image_names[i].substring(0, image_names[i].length()-4)+".json");
        
        //create a new entry in the complete face data file
        all_face_data.setJSONArray(image_names[i], image_faces);
      }
    }
    catch(Exception e) {
      println("exception: "+image_names[i]);
    }
